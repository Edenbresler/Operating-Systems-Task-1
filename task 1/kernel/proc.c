...
fork(void)
{
  ...
  // Our code:
  //copy parent's mask (CPUs) to child
...
exit(int status, char *exit_msg)
{
  ...
  safestrcpy(p->exit_msg, "No exit message", sizeof(p->exit_msg));
    ...
    safestrcpy(p->exit_msg, exit_msg, sizeof(p->exit_msg));  //copyy 2
  }
  p->xstate = status;
  p->state = ZOMBIE;
    ...
    for(pp = proc; pp < &proc[NPROC]; pp++){
      if(pp->parent == p){
        ...
        if(pp->state == ZOMBIE){
          ...
          //Our code:
          // copy the exit massage from the child to the parent
          if (exit_msg != 0) { //check if the pointer "exit_msg" is not null
            //copy pp->exit_msg to exit_msg(the parent) that in th p->pagetable
            if (copyout(p->pagetable, (uint64)exit_msg, (char *)pp->exit_msg, sizeof(pp->exit_msg)) < 0) {  //copyy 3 
              release(&pp->lock);
              release(&wait_lock);
              return -1;
            }
...
//Our code:
void update_effective(struct proc *p){
  if (p->affinity_mask != 0) {
    
    p->effective_affinity_mask &= ~(1 << cpuid()); // "~" turn the bit
